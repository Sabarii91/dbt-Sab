...
{% macro run_validation_tests(
  ...
  {% if execute and config_result and config_result.rows | length > 0 %}
    ...
    {% for row in config_result.rows %}
      ...
      {% set duplicate_no_match_count = 0 %}
      {% set duplicate_partial_match_count = 0 %}
      {% set duplicate_full_match_count = 0 %}
      ...
      {% if test_type == 'comprehensive_compare' %}
        ...
        {% set compare_failures_subquery %}
          ...
          where anomaly_type not in (
            ...
            'DUPLICATE_NO_MATCH_INFO',
            'DUPLICATE_PARTIAL_MATCH_INFO'
          )
        {% endset %}
        ...
        {% set anomaly_count_sql %}
          select
            sum(case when anomaly_type not in ('DUPLICATE_FULL_MATCH', 'DUPLICATE_NO_MATCH_INFO', 'DUPLICATE_PARTIAL_MATCH_INFO') then 1 else 0 end) as total_anomalies_count,
            ...
            sum(case when anomaly_type = 'DUPLICATE_NO_MATCH' then 1 else 0 end) as duplicate_no_match_count,
            sum(case when anomaly_type = 'DUPLICATE_PARTIAL_MATCH' then 1 else 0 end) as duplicate_partial_match_count,
            sum(case when anomaly_type = 'DUPLICATE_FULL_MATCH' then 1 else 0 end) as duplicate_full_match_count
        ...
        {% set create_failures_view_sql %}
          ...
          select
            ...
            , {{ adapter.quote('source_' ~ col) }}
            , {{ adapter.quote('target_' ~ col) }}
            {% endfor %}
          from {{ failures_table }}
        {% endset %}
        ...
        {% if anomaly_count_res and anomaly_count_res.rows | length > 0 %}
          ...
          {% set duplicate_no_match_count = anomaly_count_res.rows[0][4] or 0 %}
          {% set duplicate_partial_match_count = anomaly_count_res.rows[0][5] or 0 %}
          {% set duplicate_full_match_count = anomaly_count_res.rows[0][6] or 0 %}
        {% endif %}
      ...
      {% else %}
        {% do exceptions.raise_compiler_error(
          ...
          ~ '. Allowed: comprehensive_compare, delta_compare.'
        ) %}
      {% endif %}
      ...
      {% set audit_insert_sql %}
        insert into {{ qa_prefix }}validation_audit_log (
          ...
          rows_compared, anomaly_count, delta_new_count, delta_updated_count, execution_duration_ms, error_message
        )
        values (
          '{{ audit_run_id }}',
          '{{ execution_id }}',
